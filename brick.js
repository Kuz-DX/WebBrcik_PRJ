...
class FBomb {
    ...
    draw(ctx) {
        ...
        ctx.font = "bold 56px 'Galmuri11', sans-serif";
...
class Brick {
    ...
    onHit(damage = 1) { //블록 쳤을때 기능 함수 실행 //status 맵 활용 추가
        ...
        // 함수를 실행할 때 this가 무조건 현재 벽돌 가리키도록
...
function drawTopUI() {
    ...
    if (progressText !== "") {
        ...
        ctx.lineWidth = 3;
        ctx.strokeStyle = "#000000";
        ctx.strokeText(progressText, barX - 15, 20);
        ctx.fillStyle = "#FFFFFF";
...
function getCalculatedScore() {
    ...
    switch(currentStage) {
        ...
        case 2: cutlines = [55, 65, 80, 90, 110]; break; //객지프
...
function endGame(message, isAllClear = false) {
    ...
    if (isAllClear) {
        ...
        if (finalScoreBoard) finalScoreBoard.style.display = "flex";
...
function clearGame(){
    ...
    let prevRecord = highestGrades[currentStage];
    // 더 높은 난이도, 같은 난이도에서 학점이 더 높을 때 갱신
    if (!prevRecord || currentRecordScore > prevRecord.score) {
